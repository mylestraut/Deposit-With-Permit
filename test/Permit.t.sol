...
contract permitTest is Test {
    ...
    function test_Permit() public {
        ...
        // Digest is the hash of the domain separater + the preconfigured hash of the permit struct
        bytes32 digest = sigUtils.getTypedDataHash(permit);

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ownerPrivateKey, digest);

        //console.logUint(token.allowance(owner, spender));
        assertEq(token.allowance(owner, spender), 0);
        //console.logUint(token.nonces(owner));
        assertEq(token.nonces(owner), 0);

        // set spender allowance using permit
        token.permit(
            permit.owner,
            permit.spender,
            permit.value,
    ...
    function testRevert_ExpiredPermit() public {
        ...
        //Move time forward 1 day + 1 second
    ...
    function testRevert_InvalidNonce() public {
        SigUtils.Permit memory permit = SigUtils.Permit({
            ...
            spender: spender,
            value: 1e18,
            nonce: 1, // owner nonce on chain is 0
            deadline: 1 days
        });
    ...
    function testFail_InvalidBalance() public {
        SigUtils.Permit memory permit = SigUtils.Permit({
            owner: owner,
            spender: spender,
            value: 2e18, // Approve more than what owner has
            nonce: 0,
            deadline: 1 days
        });
        ...
        // Try transfer 2 tokens
        token.transferFrom(owner, spender, 2e18);
    }
}
